% 1. Load the PNG image (Full Image)
img_filename = "../_images/flow_snaps/post1_OG.png"; 
img = imread(img_filename);

% Ensure it is grayscale (if it's RGB)
if size(img, 3) == 3
    img = rgb2gray(img); 
end
...
figure;
imshow(img, []); % Displaying the FULL image now
hold on;

num_lines = 30;                     
H = size(img, 1); % Get height of the full image
...
for i = 1:num_lines
    ...
    % Using 'img' instead of 'single_cap'
...
title(sprintf('Sampling lines (n=%d) on Full Image', num_lines)); axis on;
...
writetable(T_wide, 'flow_intensity_profiles_wide_full.csv');
writetable(T_mean, 'flow_intensity_profile_mean_full.csv');